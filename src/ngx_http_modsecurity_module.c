...
ngx_http_modsecurity_create_ctx(ngx_http_request_t *r)
{
    ...
    if (ctx == NULL)
    {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "failed to allocate memory for the context.");
        return NULL;
    }
    ...
    ngx_log_error(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "creating transaction with the following rules: '%p' -- ms: '%p'", mcf->rules_set, mmcf->modsec);
    ...
    ngx_log_error(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "transaction created");
    ...
    if (cln == NULL)
    {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "failed to create the ModSecurity context cleanup");
        return NGX_CONF_ERROR;
    }
...
char *
ngx_conf_modsecurity_set_rules(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    int res;
    char *rules;
    ngx_str_t *value;
    ...
    if (res < 0)
    {
        ngx_log_error(NGX_LOG_ERR, cf->log, 0, "Failed to load the rules: '%s' - reason: '%s'", rules, error);
        return strdup(error);
    }
...
char *
ngx_conf_modsecurity_set_rules_file(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    int res;
    char *rules_set;
    ngx_str_t *value;
    ...
    if (res < 0)
    {
        ngx_log_error(NGX_LOG_ERR, cf->log, 0, "Failed to load the rules from: '%s' - reason: '%s'", rules_set, error);
        return strdup(error);
    }
...
char *
ngx_conf_modsecurity_set_rules_remote(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    int res;
    ngx_str_t *value;
    const char *error;
    ...
    if (res < 0)
    {
        ngx_log_error(NGX_LOG_ERR, cf->log, 0, "Failed to load the rules from: '%s'  - reason: '%s'", rules_remote_server, error);
        return strdup(error);
    }
...
char *ngx_conf_modsecurity_set_transaction_id(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_str_t *value;
...
ngx_http_modsecurity_init(ngx_conf_t *cf)
{
    ...
    if (cmcf == NULL)
    {
        ngx_log_error(NGX_LOG_ERR, cf->log, 0, "We are not sure how this returns, NGINX doesn't seem to think it will ever be null");
        return NGX_ERROR;
    }
    /**
     *
    ...
    if (h_rewrite == NULL)
    {
        ngx_log_error(NGX_LOG_ERR, cf->log, 0, "Not able to create a new NGX_HTTP_REWRITE_PHASE handle");
        return NGX_ERROR;
    }
    *h_rewrite = ngx_http_modsecurity_rewrite_handler;
    ...
    if (h_preaccess == NULL)
    {
        ngx_log_error(NGX_LOG_ERR, cf->log, 0, "Not able to create a new NGX_HTTP_PREACCESS_PHASE handle");
        return NGX_ERROR;
    }
    *h_preaccess = ngx_http_modsecurity_pre_access_handler;
    ...
    if (h_log == NULL)
    {
        ngx_log_error(NGX_LOG_ERR, cf->log, 0, "Not able to create a new NGX_HTTP_LOG_PHASE handle");
        return NGX_ERROR;
    }
...
ngx_http_modsecurity_create_main_conf(ngx_conf_t *cf)
{
    ...
    if (conf->modsec == NULL)
    {
        ngx_log_error(NGX_LOG_ERR, cf->log, 0, "failed to create the ModSecurity instance");
        return NGX_CONF_ERROR;
    }
    ...
    ngx_log_error(NGX_LOG_DEBUG, cf->log, 0, "main conf created at: '%p', instance is: '%p'", conf, conf->modsec);
...
ngx_http_modsecurity_create_conf(ngx_conf_t *cf)
{
    ...
    if (conf == NULL)
    {
        ngx_log_error(NGX_LOG_ERR, cf->log, 0, "Failed to allocate space for ModSecurity configuration");
        return NGX_CONF_ERROR;
    }
    ...
    if (cln == NULL)
    {
        ngx_log_error(NGX_LOG_ERR, cf->log, 0, "failed to create the ModSecurity configuration cleanup");
        return NGX_CONF_ERROR;
    }
    ...
    ngx_log_error(NGX_LOG_DEBUG, cf->log, 0, "conf created at: '%p'", conf);
...
// endif
    ...
    ngx_log_error(NGX_LOG_DEBUG, cf->log, 0, "merging loc config [%s] - parent: '%p' child: '%p'",
                  ngx_str_to_char(clcf->name, cf->pool), parent,
                  child);

    ngx_log_error(NGX_LOG_DEBUG, cf->log, 0, "                  state - parent: '%d' child: '%d'",
       (int)c->enable, (int)p->enable);

    ngx_conf_merge_value(c->enable, p->enable, 0);
    ngx_conf_merge_str_value(c->thread_pool_name, p->thread_pool_name, "");
